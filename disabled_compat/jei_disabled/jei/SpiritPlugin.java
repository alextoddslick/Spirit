...
public class SpiritPlugin implements IModPlugin {
    ...
    public ResourceLocation getPluginUid() {
        return ResourceLocation.fromNamespaceAndPath(Spirit.MODID, "jei");
    }
    ...
    public void registerRecipes(@NotNull IRecipeRegistration registration) {
        ...
        if (level != null) {
            registration.addRecipes(PedestalRecipeCategory.RECIPE,
                    level.getRecipeManager().getAllRecipesFor(SpiritMisc.SOUL_TRANSMUTATION_RECIPE.get()).stream()
                            .map(net.minecraft.world.item.crafting.RecipeHolder::value).toList());
            registration.addRecipes(SoulCageCategory.RECIPE,
                    level.getRecipeManager().getAllRecipesFor(SpiritMisc.TIER_RECIPE.get()).stream()
                            .map(net.minecraft.world.item.crafting.RecipeHolder::value).toList());
            registration.addRecipes(SoulEngulfingCategory.RECIPE,
                    SoulEngulfingCategory.getRecipes(
                            level.getRecipeManager().getAllRecipesFor(SpiritMisc.SOUL_ENGULFING_RECIPE.get()).stream()
                                    .map(net.minecraft.world.item.crafting.RecipeHolder::value).toList()));
        }
    }
    ...
    public void registerRecipeCatalysts(IRecipeCatalystRegistration registration) {
        registration.addRecipeCatalyst(SpiritBlocks.SOUL_PEDESTAL.get().asItem().getDefaultInstance(),
                PedestalRecipeCategory.RECIPE);
        registration.addRecipeCatalyst(SpiritBlocks.PEDESTAL.get().asItem().getDefaultInstance(),
                PedestalRecipeCategory.RECIPE);
        registration.addRecipeCatalyst(SpiritBlocks.SOUL_CAGE.get().asItem().getDefaultInstance(),
                SoulCageCategory.RECIPE);
