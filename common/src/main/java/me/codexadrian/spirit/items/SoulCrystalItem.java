...
public class SoulCrystalItem extends Item {
    ...
    public void appendHoverText(@NotNull ItemStack itemStack, @NotNull TooltipContext context,
        ...
        Component entityOrNone = Optional.ofNullable(SoulUtils.getSoulCrystalType(itemStack))
                ...
                .map(entityType -> Component
                        .translatable("item.spirit.soul_crystal.entity_component",
    ...
    private static List<Component> shiftToolTipComponents(@NotNull ItemStack itemStack, @Nullable Level level) {
        ...
        if (tier == null && (SoulUtils.canCrystalBeUsedInCage(itemStack) || (Services.PLATFORM.isModLoaded("vitalize")
                && SoulUtils.isAllowed(itemStack, Spirit.REVITALIZER_TAG)))) {
            list.add(
                    Component.translatable(Services.PLATFORM.isModLoaded("vitalize") ? "misc.spirit.not_viable_vitalize"
                            : "misc.spirit.not_viable").withStyle(ChatFormatting.RED));
        }
        Tier nextTier = SoulUtils.getNextTier(itemStack, level);
        if (nextTier != null) {
            list.add(Component.translatable("misc.spirit.next_tier",
                    nextTier.requiredSouls() - SoulUtils.getSoulsInCrystal(itemStack),
                    Component.translatable(nextTier.displayName())).withStyle(ChatFormatting.GRAY));
        }
