...
public record SoulfireMultiblock(List<List<String>> pattern, Map<String, StrippedBlockPredicate> keys) {
    ...
    static {
        RESERVED_VALUES.put("@", new StrippedBlockPredicate(Optional.of(HolderSet.direct(SpiritBlocks.RAGING_SOUL_FIRE.get().builtInRegistryHolder())), Optional.empty()));
        RESERVED_VALUES.put("&", new StrippedBlockPredicate(Optional.of(BuiltInRegistries.BLOCK.getOrCreateTag(BlockTags.SOUL_FIRE_BASE_BLOCKS)), Optional.empty()));
    ...
    public record StrippedBlockPredicate(Optional<HolderSet<Block>> blocks, Optional<CompoundTag> nbtTag) {
        ...
        public static final Codec<StrippedBlockPredicate> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                HolderSetCodec.of(BuiltInRegistries.BLOCK).optionalFieldOf("block").forGetter(StrippedBlockPredicate::blocks),
